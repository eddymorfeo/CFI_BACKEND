...
class BancoEstadoCuentaCorrienteParser(BaseParser):
    ...
    def parse(self, file_path: str) -> dict:
        ...
        document_metadata = self._build_document_metadata(page_metadata)
    ...
    def _build_document_metadata(self, page_metadata: list[dict]) -> dict:
        date_from_values = [
            metadata["document_date_from"]
            for metadata in page_metadata
            if metadata.get("document_date_from") is not None
        ]
        date_to_values = [
            metadata["document_date_to"]
            for metadata in page_metadata
    ...
    def _extract_movements(self, pages: list[dict], page_metadata: list[dict]) -> list[dict]:
        ...
        for page, metadata in zip(pages, page_metadata):
            ...
            previous_balance = metadata.get("previous_balance") or Decimal("0")
            date_from = metadata.get("document_date_from")
            date_to = metadata.get("document_date_to")
            in_movements = False
